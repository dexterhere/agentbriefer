//! Detects an F# project from a `*.fsproj` file. Same XML shape as C#'s
//! `*.csproj` and the same NuGet ecosystem, so it reuses `csharp`'s
//! framework/testing/database lookup lists rather than duplicating them.
...
pub(super) fn detect(root: &Path) -> Option<DetectedStack> {
    let manifest_path = find_file_with_extension(root, "fsproj")?;
    ...
    Some(DetectedStack {
        language: Some("fsharp".to_string()),
...
"#,
        ...
        assert_eq!(detected.language.as_deref(), Some("fsharp"));
        assert_eq!(detected.package_manager.as_deref(), Some("nuget"));
        assert_eq!(detected.testing_tools, vec!["xunit".to_string()]);
        assert_eq!(detected.source.as_deref(), Some("Example.fsproj"));
    }

    #[test]
    fn returns_none_when_no_fsproj() {
        let dir = tempfile::tempdir().unwrap();
